...
(async()=>{
  console.log('AT-MEC_HM_7.7.1 startup doctor');
  ...
  check('AI Provider Approval 7.7.1 script presente', exists('src/renderer/js/modules/ai/ai-provider-approval-77.js'));
  check('AI Provider Approval 7.7.1 CSS presente', exists('src/renderer/css/modules/35-ai-provider-approval-77.css'));
  ...
  const criticalMissing=!exists('package.json')||!exists('dist/main/main.js')||!exists('src/renderer/index.html')||!exists('src/renderer/js/modules/ai/ai-copilot-76.js')||!exists('src/renderer/js/modules/ai/ai-provider-approval-77.js');
  const electronMissing=!electronBinaryExists();
  const report={version:'AT-MEC_HM_7.7.1_AI_APPROVAL_PERSISTENCE_UX',createdAt:new Date().toISOString(),checks,criticalMissing,electronMissing};
  try{fs.mkdirSync(path.join(root,'docs/quality'),{recursive:true});fs.writeFileSync(path.join(root,'docs/quality/AT_MEC_HM_7_7_1_STARTUP_DOCTOR.json'),JSON.stringify(report,null,2));}catch(_e){}
