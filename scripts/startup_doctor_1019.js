...
let ok=true; console.log('VEXON 10.1.9 startup doctor');
for(const p of required){const e=fs.existsSync(path.join(root,p)); console.log((e?'OK  ':'FAIL')+p); if(!e)ok=false;}
try{const pkg=JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8')); console.log('VERSION '+pkg.version+' '+pkg.name); if(pkg.version!=='10.1.9')ok=false;}catch(e){ok=false;}
