...
add('stable live popup JS',exists('src/renderer/js/modules/ui/action-live-measurement-1012.js'),'live popup');
...
const report={version:'AT-MEC_HM_10.1.7_STABLE_MEASUREMENT_LIVE_POPUP_FAST_UI_ENGINE_FIX',createdAt:new Date().toISOString(),checks,criticalMissing:critical,electronMissing};
fs.mkdirSync(path.join(root,'docs/quality'),{recursive:true}); fs.writeFileSync(path.join(root,'docs/quality/AT_MEC_HM_10_1_7_STARTUP_DOCTOR.json'),JSON.stringify(report,null,2));
console.log('AT-MEC_HM_10.1.7 startup doctor'); checks.forEach(c=>console.log(`${c.ok?'OK  ':'WARN'} ${c.label}${c.detail?' - '+c.detail:''}`)); if(critical)process.exit(2); if(electronMissing)process.exit(3); process.exit(0);
