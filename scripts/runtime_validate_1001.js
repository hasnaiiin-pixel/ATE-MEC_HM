...
const RELEASE='AT-MEC_HM_10.0.1_LOGIN_USERS_VISIBLE_FIX';
...
add('package 10.0.1',pkg.version==='10.0.1' && /10-0-1-login-users-visible-fix/.test(pkg.name||''),`${pkg.name} ${pkg.version}`);
add('package-lock 10.0.1',lock.version==='10.0.1' && lock.packages && lock.packages[''] && lock.packages[''].version==='10.0.1','package-lock root');
add('runtime script 10.0.1',pkg.scripts && pkg.scripts['runtime:validate']==='node scripts/runtime_validate_1001.js','package script');
add('startup doctor 10.0.1',pkg.scripts && pkg.scripts['startup:doctor']==='node scripts/startup_doctor_1001.js','package script');
...
add('BAT 10.0.1 presenti',exists('INSTALLA_AT_MEC_HM_10.0.1_LOGIN_USERS_VISIBLE_FIX.bat')&&exists('AVVIA_AT_MEC_HM_10.0.1_LOGIN_USERS_VISIBLE_FIX.bat')&&exists('CREA_INSTALLER_WINDOWS_10.0.1_LOGIN_USERS_VISIBLE_FIX.bat'),'install/avvia/installer');
add('index 10.0.1',/10\.0\.1 LOGIN USERS VISIBLE FIX/.test(text('src/renderer/index.html')),'title');
add('version.js 10.0.1',/AT-MEC_HM_10\.0\.1_LOGIN_USERS_VISIBLE_FIX/.test(text('src/renderer/js/version.js')),'version label');
...
const score=Math.round(checks.filter(c=>c.ok).length/checks.length*100); const report={version:RELEASE,createdAt:new Date().toISOString(),score,checks}; fs.mkdirSync(path.join(root,'docs/quality'),{recursive:true}); fs.writeFileSync(path.join(root,'docs/quality/AT_MEC_HM_10_0_1_RUNTIME_VALIDATION.json'),JSON.stringify(report,null,2));
console.log('AT-MEC_HM_10.0.1 runtime validation'); checks.forEach(c=>console.log(`${c.ok?'OK  ':'FAIL'} ${c.label}${c.detail?' - '+c.detail:''}`)); console.log(`SCORE ${score}%`); if(checks.some(c=>!c.ok))process.exit(1);
