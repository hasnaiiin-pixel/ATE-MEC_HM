...
const RELEASE='AT-MEC_HM_10.1.9_STABLE_MEASUREMENT_OPERATOR_SPEED_AUTOSTART_FIX';
...
add('package VEXON 10.1.9',pkg.version==='10.1.9' && pkg.name==='vexon-industrial-test-platform-10-1-9',`${pkg.name} ${pkg.version}`);
add('package-lock 10.1.9',lock.version==='10.1.9' && lock.packages && lock.packages[''] && lock.packages[''].version==='10.1.9','package-lock root');
add('runtime script 10.1.9',pkg.scripts && pkg.scripts['runtime:validate']==='node scripts/runtime_validate_1019.js','package script');
add('startup doctor 10.1.9',pkg.scripts && pkg.scripts['startup:doctor']==='node scripts/startup_doctor_1019.js','package script');
...
add('Clean popup 10.1.9',/vx1018-live-popup/.test(actionJs)&&/MISURA ATTESA/.test(actionJs)&&/MISURA MULTIMETRO LIVE/.test(actionJs),'clean popup');
add('Retry multimetro',/retryStableMeasurement1018/.test(actionJs)&&/RIPROVA MULTIMETRO/.test(actionJs),'retry button');
add('Manual fallback in same popup',/submitManualFallback1018/.test(actionJs)&&/FALLBACK MANUALE/.test(actionJs),'manual same popup');
...
add('BAT VEXON 10.1.9 presenti',exists('AVVIA_VEXON_10.1.9.bat')&&exists('INSTALLA_VEXON_10.1.9.bat')&&exists('CREA_INSTALLER_WINDOWS_VEXON_10.1.9.bat'),'bat');
...
const report={version:RELEASE,createdAt:new Date().toISOString(),score,checks}; fs.mkdirSync(path.join(root,'docs/quality'),{recursive:true}); fs.writeFileSync(path.join(root,'docs/quality/AT_MEC_HM_10_1_9_RUNTIME_VALIDATION.json'),JSON.stringify(report,null,2));
console.log('AT-MEC_HM_10.1.9 runtime validation'); checks.forEach(c=>console.log(`${c.ok?'OK  ':'FAIL'} ${c.label}${c.detail?' - '+c.detail:''}`)); console.log(`SCORE ${score}%`); if(checks.some(c=>!c.ok))process.exit(1);
