...
const RELEASE='AT-MEC_HM_10.1.2_ACTION_LIVE_MEASUREMENT_RETRY';
...
const actionJs=text('src/renderer/js/modules/ui/action-live-measurement-1012.js'), actionCss=text('src/renderer/css/modules/45-action-live-measurement-1012.css'), preload=text('src/main/preload.ts'), preloadJs=text('dist/main/preload.js'), mainTs=text('src/main/main.ts'), mainJs=text('dist/main/main.js');
add('package VEXON 10.1.2',pkg.version==='10.1.2' && pkg.name==='vexon-industrial-test-platform-10-1-2',`${pkg.name} ${pkg.version}`);
add('package-lock 10.1.2',lock.version==='10.1.2' && lock.packages && lock.packages[''] && lock.packages[''].version==='10.1.2','package-lock root');
add('runtime script 10.1.2',pkg.scripts && pkg.scripts['runtime:validate']==='node scripts/runtime_validate_1012.js','package script');
add('startup doctor 10.1.2',pkg.scripts && pkg.scripts['startup:doctor']==='node scripts/startup_doctor_1012.js','package script');
...
add('BAT VEXON 10.1.2 presenti',exists('AVVIA_VEXON_10.1.2.bat')&&exists('INSTALLA_VEXON_10.1.2.bat')&&exists('CREA_INSTALLER_WINDOWS_VEXON_10.1.2.bat'),'bat');
const score=Math.round(checks.filter(c=>c.ok).length/checks.length*100); const report={version:RELEASE,createdAt:new Date().toISOString(),score,checks}; fs.mkdirSync(path.join(root,'docs/quality'),{recursive:true}); fs.writeFileSync(path.join(root,'docs/quality/AT_MEC_HM_10_1_2_RUNTIME_VALIDATION.json'),JSON.stringify(report,null,2));
console.log('AT-MEC_HM_10.1.2 runtime validation'); checks.forEach(c=>console.log(`${c.ok?'OK  ':'FAIL'} ${c.label}${c.detail?' - '+c.detail:''}`)); console.log(`SCORE ${score}%`); if(checks.some(c=>!c.ok))process.exit(1);
