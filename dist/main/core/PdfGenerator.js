...
class PdfGenerator {
    static APP_VERSION = '4.12I';
    static safeFilePart(value) {
        return String(value || 'ND').trim().replace(/[^a-zA-Z0-9_\-]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 80) || 'ND';
    }
    static text(value, fallback = 'N/D') {
        if (value === null || value === undefined || value === '')
            return fallback;
        return String(value);
    }
    static numericText(value, unit = '') {
        if (value === null || value === undefined || value === '')
            return 'N/D';
    ...
    static resolveCustomerLogo(report) {
    ...
    static drawHeader(doc, title, subtitle, resultColor, report) {
        ...
        const customerName = PdfGenerator.text(report?.customer_name || report?.client_name || report?.customer, '');
        ...
        try {
            if (reportLarge && fs.existsSync(reportLarge))
                doc.image(reportLarge, 40, 30, { fit: [168, 56] });
        }
        catch { }
        try {
            if (reportSmall && fs.existsSync(reportSmall))
                doc.image(reportSmall, 394, 28, { fit: [154, 58], align: 'right' });
        }
        catch { }
        if (customerLogo) {
            try {
                doc.roundedRect(232, 24, 128, 54, 8).fill('#ffffff').stroke('#e4e7ee');
                doc.image(customerLogo, 240, 30, { fit: [112, 42], align: 'center' });
            }
            catch { }
        }
    ...
    static drawKeyValue(doc, x, y, label, value, width = 240) {
    ...
    static ensureSpace(doc, yPos, needed = 44) {
        if (yPos + needed > 790) {
    ...
    static measurementSummary(step) {
        ...
        if (target)
            parts.push(`Atteso: ${target}`);
        if (min || max)
            parts.push(`Range: ${min || 'N/D'} → ${max || 'N/D'}`);
        if (tol)
            parts.push(`Tol: ${tol}`);
        return parts.join('\n');
    }
    static generateCertificate(report) {
        ...
        const lotPart = PdfGenerator.safeFilePart(report.lot_number || report.work_order || 'NOLOTTO');
        ...
        PdfGenerator.drawHeader(doc, 'AT-MEC HM - TEST REPORT', 'Certificato di collaudo, misure universali e tracciabilità prodotto', resultColor, report);
        ...
        PdfGenerator.drawKeyValue(doc, 205, 250, 'Commessa / Lotto', report.lot_number || report.work_order || 'N/D', 135);
        PdfGenerator.drawKeyValue(doc, 54, 286, 'Postazione', report.station_name || report.station_id || 'N/D', 140);
        PdfGenerator.drawKeyValue(doc, 205, 286, 'Station ID / Sede', [(report.station_id || ''), (report.station_site || '')].filter(Boolean).join(' / ') || 'N/D', 135);
        ...
        if (report.repair_note) {
            ...
            doc.font('Helvetica').fontSize(9).fillColor('#431407').text(String(report.repair_note), 54, yPos + 22, { width: 482, height: 16, ellipsis: true });
            yPos += 58;
        }
