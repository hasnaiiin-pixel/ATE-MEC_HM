...
exports.Esp32SerialProvider = void 0;
/**
...
class Esp32SerialProvider {
    ...
    async connect(path, baudRate = 115200) {
        ...
        if (!hello?.ok)
            throw new Error('Handshake ESP32 JSON non valido');
    }
    isConnected() {
     ...
     */
    async request(cmd, payload, timeoutMs = 1500) {
        if (!this.port?.isOpen)
            throw new Error('ESP32 seriale non connesso');
    ...
    handleLine(line) {
        try {
            ...
            this.pending.delete(id);
            if (msg.ok === false)
                pending.reject(new Error(msg.error || 'Errore ESP32'));
            else
                pending.resolve(msg);
        }
        catch (err) {
...
exports.Esp32SerialProvider = Esp32SerialProvider;
