...
const IotServer_1 = require("./core/IotServer");
const PdfGenerator_1 = require("./core/PdfGenerator");
const AuditSystem_1 = require("./core/AuditSystem");
const LocalDatabase_1 = require("./core/LocalDatabase");
const DataProvider_1 = require("./core/DataProvider");
const DeviceManager_1 = require("./hal/DeviceManager");
const RecipeEngine_1 = require("./runtime/RecipeEngine");
let mainWindow = null;
let iotServer;
let hal;
let recipeEngine;
...
let localDb;
let dataProvider;
let activeRunTraceMeta412I = {};
let commSerial = null;
let commTcp = null;
let commBuffer = [];
let commType = null;
function pushComm(dir, data) {
    const row = { ts: new Date().toISOString(), dir, data: String(data ?? '') };
    commBuffer.push(row);
    if (commBuffer.length > 1000)
        commBuffer = commBuffer.slice(-1000);
    try {
        mainWindow?.webContents.send('comm-rx', row);
    }
    catch { }
}
function closeCommunicationHub() {
    try {
...
let kpiPassed = 0;
let kpiFailed = 0;
function timeoutPromise(task, ms, label) {
    let timer;
    return Promise.race([
        task,
...
function classifySettingsPermissions(settings) {
    ...
    for (const k of keys) {
        if (brandingRe.test(k))
            required.add('manage_branding');
        if (hardwareRe.test(k))
            required.add('config_hardware');
    }
    return Array.from(required);
}
function isTcpConnectionText(conn) {
    const clean = String(conn || '').trim().replace(/^tcp:\/\//i, '');
    return /^\d+\.\d+\.\d+\.\d+(:\d+)?$/.test(clean) || /^[a-zA-Z0-9_.-]+:\d+$/.test(clean);
}
function settingsFromReconnectConfig(c) {
    const name = String(c?.name || '').trim();
    const conn = String(c?.conn || '').trim();
    const baud = Number(c?.baud || 0);
    if (!name || !conn || conn === 'mock')
        return {};
...
function persistReconnectSettings(configs) {
    const merged = (configs || []).reduce((acc, c) => ({ ...acc, ...settingsFromReconnectConfig(c) }), {});
    if (Object.keys(merged).length)
        writeAppSettings(merged);
}
function normalizeInstrumentName(name) {
    ...
    if (n.includes('scanner') || n.includes('qr'))
        return 'QR_Scanner';
    if (['manual', 'manuale', 'operator', 'system', 'none'].includes(n))
        return n;
    return String(name || '').trim();
}
function isInstrumentExcluded(name, excluded) {
...
function initSystems() {
    ...
    auditSystem = new AuditSystem_1.AuditSystem();
    localDb = new LocalDatabase_1.LocalDatabase();
    dataProvider = new DataProvider_1.DataProvider(localDb);
    stateMachine = new StateMachine_1.StateMachine((newState) => {
    ...
    hal = new DeviceManager_1.DeviceManager();
    recipeEngine = new RecipeEngine_1.RecipeEngine(stateMachine, eventBus, hal);
    iotServer = new IotServer_1.IotServer(8080);
    ...
    eventBus.subscribe('run_completed', (data) => {
        const baseReport = data.report || null;
        ...
        if (report) {
            try {
                dataProvider.saveTestReport(report);
            }
            catch (err) {
                console.error('[DATA_PROVIDER] save report:', err);
            }
        }
        if (report)
            PdfGenerator_1.PdfGenerator.generateCertificate(report);
        kpiTotal++;
        if (report)
            iotServer.updateLiveKpi(kpiTotal, kpiPassed, kpiFailed, report.serial_dut, report.final_result);
...
async function connectHardware() {
    ...
    const excluded = new Set(Array.isArray(cfg.excludedInstruments) ? cfg.excludedInstruments : []);
...
electron_1.app.on('window-all-closed', async () => {
    try {
        await timeoutPromise(hal.safePl303AllOutputsOff('APP_CLOSE'), 2500, 'PL303 off chiusura app');
    }
    catch { }
...
electron_1.app.on('before-quit', async () => {
    try {
        await timeoutPromise(hal.safePl303AllOutputsOff('APP_BEFORE_QUIT'), 2500, 'PL303 off before quit');
    }
    catch { }
});
...
safeIpcHandle('print420a5-print-job', async (_e, job) => {
...
safeIpcHandle('connect-esp32-only', async (_e, cfg) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
    const port = String(cfg?.port || '').trim();
    const baud = Number(cfg?.baud || 115200);
    if (!port || port === 'mock')
        return { ok: false, error: 'Porta ESP32 non selezionata', statuses: hal.getAllStatuses() };
    ...
    if (st && !st.mock)
        writeAppSettings(settingsFromReconnectConfig({ name: 'modbus_serial', conn: port, baud }));
    return { ok: !!st && !st.mock, status: st, statuses };
});
safeIpcHandle('reconnect-hardware', async (_e, configs) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
...
safeIpcHandle('start-test', async (_e, payload) => {
    const denied = requirePermission('run_test');
    if (denied)
        return denied;
    ...
    activeRunTraceMeta412I = {
        customerName: recipe.client_name || recipe.customer || '',
        customerLogo: recipe.customer_logo || recipe.client_logo || '',
        productName: recipe.product_name || recipe.product || ''
    };
...
safeIpcHandle('stop-test', async () => {
    ...
    try {
        recipeEngine.requestStop();
    }
    catch { }
    try {
        await timeoutPromise(hal.safePl303AllOutputsOff('STOP_TEST'), 2500, 'PL303 OFF stop test');
    }
    catch { }
    try {
        recipeEngine.forceResetAfterStop?.();
    }
    catch { }
...
safeIpcHandle('emergency-stop-all', async () => {
    ...
    try {
        ...
        errors.push('STOP test: ' + (err?.message || String(err)));
    }
    try {
        await timeoutPromise(hal.safePl303AllOutputsOff('EMERGENZA'), 7000, 'PL303 OFF emergenza');
    }
    catch (err) {
        errors.push('PL303 OFF emergenza: ' + (err?.message || String(err)));
    }
    ...
    try {
        mainWindow?.webContents.send('hardware-statuses', hal.getAllStatuses());
    }
    catch { }
    try {
        mainWindow?.webContents.send('state-changed', stateMachine.getState());
...
safeIpcHandle('set-debug-mode', (_e, enabled) => {
    const denied = requirePermission('debug_mode');
    if (denied)
        return denied;
...
safeIpcHandle('failure-action', (_e, action) => {
...
safeIpcHandle('flash-firmware', async (_e, { tool, operation, filePath }) => {
    ...
    if (denied)
        return denied;
    const { FlashManager } = await Promise.resolve().then(() => __importStar(require('./hal/FlashManager')));
...
safeIpcHandle('connect-pl303', async (_e, cfg) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
...
safeIpcHandle('set-pl303-output', async (_e, cfg) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
...
safeIpcHandle('query-pl303-status', async (_e, channel = 1) => {
    try {
...
safeIpcHandle('safe-pl303-off', async (_e, reason = 'MANUAL_SAFE_OFF') => {
    try {
...
safeIpcHandle('measure-pl303-current', async (_e, channel = 1) => {
    try {
...
safeIpcHandle('read-digital-input', async (_e, channel) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
    return timeoutPromise(hal.readDigitalInput(Number(channel) || 0), 2000, 'read DI');
});
safeIpcHandle('read-digital-output', async (_e, channel) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
    return timeoutPromise(hal.readDigitalOutput(Number(channel) || 0), 2000, 'read DO');
});
safeIpcHandle('read-analog-input', async (_e, channel) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
...
safeIpcHandle('comm-open-serial', async (_e, cfg) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
    closeCommunicationHub();
    ...
    if (!port)
        return { ok: false, error: 'Porta seriale non selezionata' };
    commType = 'serial';
    commBuffer = [];
    commSerial = new serialport_1.SerialPort({ path: port, baudRate, autoOpen: false });
    await new Promise((resolve, reject) => commSerial.open(err => err ? reject(err) : resolve()));
    commSerial.on('data', (buf) => pushComm('RX', buf.toString('utf8')));
    commSerial.on('error', (err) => pushComm('SYS', `SERIAL ERROR: ${err.message}`));
...
safeIpcHandle('comm-open-tcp', async (_e, cfg) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
    closeCommunicationHub();
    ...
    if (!host)
        return { ok: false, error: 'IP/host non inserito' };
    ...
    commTcp.on('data', (buf) => pushComm('RX', buf.toString('utf8')));
    commTcp.on('error', (err) => pushComm('SYS', `TCP ERROR: ${err.message}`));
...
safeIpcHandle('comm-send', async (_e, payload) => {
    const denied = requirePermission('config_hardware');
    if (denied)
        return denied;
    ...
    if (commSerial?.isOpen) {
        await new Promise((resolve, reject) => commSerial.write(data, err => err ? reject(err) : resolve()));
        pushComm('TX', data);
        return { ok: true };
    }
...
safeIpcHandle('comm-read-log', async () => { const denied = requirePermission('config_hardware'); if (denied)
    return denied; return { ok: true, type: commType, rows: commBuffer.slice(-500) }; });
safeIpcHandle('get-audit-history', (_e, filters) => { const denied = requirePermission('view_reports'); if (denied)
    return denied; return auditSystem.filterHistory(filters || {}).slice(-1000); });
safeIpcHandle('check-serial-history', (_e, { serialDut, lotNumber }) => { const denied = requirePermission('view_traceability'); if (denied)
    return denied; return auditSystem.findBySerialAndLot(serialDut, lotNumber); });
safeIpcHandle('get-local-db-stats', (_e, filters) => { const denied = requirePermission('view_kpi'); if (denied)
    return denied; return dataProvider.getStats(filters || {}); });
safeIpcHandle('get-serial-history', (_e, { serialDut, lotNumber }) => { const denied = requirePermission('view_traceability'); if (denied)
    return denied; return dataProvider.getSerialHistory(serialDut, lotNumber); });
safeIpcHandle('add-repair-record', (_e, payload) => { const denied = requirePermission('view_traceability'); if (denied)
    return denied; return dataProvider.addRepairRecord({ ...(payload || {}), operator: userManager.getCurrentOperator() }); });
safeIpcHandle('delete-recipe', async (_e, name) => {
    const denied = requirePermission('edit_recipe');
    if (denied)
        return denied;
    const role = String(userManager.getCurrentUser?.()?.role || userManager.getCurrentOperator?.() || '').toLowerCase();
    ...
    if (!safeName)
        return { ok: false, error: 'Nome ricetta non valido.' };
...
safeIpcHandle('list-recipe-versions', (_e, name) => { const denied = requirePermission('view_reports'); if (denied)
    return denied; return dataProvider.listRecipeVersions(name); });
safeIpcHandle('load-recipe-version', (_e, { name, version }) => {
    const denied = requirePermission('view_reports');
    if (denied)
        return denied;
...
safeIpcHandle('export-local-database', async () => {
    const denied = requirePermission('manage_data');
    if (denied)
        return denied;
...
safeIpcHandle('backup-local-database', async (_e, label) => { const denied = requirePermission('manage_data'); if (denied)
    return denied; return dataProvider.backupSnapshot(label || 'manuale'); });
safeIpcHandle('get-data-provider-status', () => { const denied = requireAnyPermission(['manage_data', 'config_hardware', 'view_kpi']); if (denied)
    return denied; return dataProvider.getStatus(); });
safeIpcHandle('get-station-trace-info', () => { const denied = requireAnyPermission(['view_traceability', 'view_kpi', 'manage_data']); if (denied)
    return denied; return dataProvider.getStationTraceInfo(); });
safeIpcHandle('save-data-provider-config', (_e, cfg) => { const denied = requireAnyPermission(['manage_data', 'config_hardware']); if (denied)
    return denied; return dataProvider.updateConfig(cfg || {}); });
safeIpcHandle('test-data-provider-server', (_e, url) => { const denied = requireAnyPermission(['manage_data', 'config_hardware']); if (denied)
    return denied; return dataProvider.testServerConnection(url); });
safeIpcHandle('sync-data-provider-now', async () => { const denied = requirePermission('manage_data'); if (denied)
    return denied; return dataProvider.syncNow(); });
safeIpcHandle('get-sync-queue-preview', (_e, limit) => { const denied = requirePermission('manage_data'); if (denied)
    return denied; return dataProvider.getQueuePreview(Number(limit || 30)); });
...
safeIpcHandle('clear-synced-sync-queue', () => { const denied = requirePermission('manage_data'); if (denied)
    return denied; return dataProvider.clearSyncedItems(); });
...
safeIpcHandle('migrate-enterprise-database', () => {
    ...
    if (denied)
        return denied;
    let authData = { users: [], roles: [] };
    try {
        const usersPath = path.join(process.cwd(), 'config', 'users.json');
        if (fs.existsSync(usersPath))
            authData = JSON.parse(fs.readFileSync(usersPath, 'utf8'));
    }
    catch { }
...
safeIpcHandle('export-enterprise-database', async () => {
    ...
    if (denied)
        return denied;
    const save = await electron_1.dialog.showSaveDialog(mainWindow, {
...
safeIpcHandle('load-recipe', async (_e, name) => {
    const fromDb = dataProvider.loadRecipe(name);
    if (fromDb)
        return { ok: true, recipe: fromDb };
...
safeIpcHandle('list-recipes', () => {
    const names = new Set();
    for (const r of dataProvider.listRecipes())
...
safeIpcHandle('reset-default-logos', () => { const denied = requirePermission('manage_branding'); if (denied)
    return denied; return { ok: true, settings: writeAppSettings(getDefaultLogoSettings()) }; });
safeIpcHandle('save-app-settings', (_e, settings) => {
    const required = classifySettingsPermissions(settings);
    for (const perm of required) {
        const denied = requirePermission(perm);
        if (denied)
            return denied;
    }
    return { ok: true, settings: writeAppSettings(settings) };
});
safeIpcHandle('select-logo-file', async (_e, kind) => {
    const denied = requirePermission('manage_branding');
    if (denied)
        return denied;
...
safeIpcHandle('select-customer-logo-file-413c', async (_e, customerName) => {
    const denied = requirePermission('manage_branding');
    if (denied)
        return denied;
...
safeIpcHandle('select-signature-file-413c', async (_e, username) => {
    const denied = requirePermission('manage_branding');
    if (denied)
        return denied;
