...
(function(){
  ...
  function bestTarget(e){
    var el=e.target; if(!el||skipUi(el)) return null;
    if(el.closest){
...
(function(){
  ...
  function loadNamed(){ var sel=document.getElementById('atmec-lm-select-362'); var name=sel&&sel.value; if(!name){ status('nessun layout selezionato'); return; } var profiles=getProfiles(); if(!profiles[name]){ status('layout non trovato'); return; } setLayout(profiles[name].layout||{}); status('caricato: '+name+' - riavvio vista'); setTimeout(function(){location.reload();},250); }
  ...
  function importFile(file){ if(!file) return; var r=new FileReader(); r.onload=function(){ try{ var data=JSON.parse(String(r.result||'{}')); var layout=data.layout || (data.profile&&data.profile.layout) || data; if(typeof layout==='string') layout=parseJson(layout,{}); if(!isPlainObject(layout)) layout={}; var name=data.name || file.name.replace(/\.json$/i,'') || nowName(); var profiles=getProfiles(); profiles[name]={created:new Date().toISOString(),layout:layout}; setProfiles(profiles); refreshSelect(); var sel=document.getElementById('atmec-lm-select-362'); if(sel) sel.value=name; status('importato: '+name); }catch(e){ status('errore import: '+(e&&e.message?e.message:e)); } }; r.readAsText(file); }
