...
function printTraceabilitySerialHistory(){
  ...
  html += '</tbody></table><p style="margin-top:22px;font-size:11px">Generato da AT-MEC HM 4.12</p></body></html>';
  const w=window.open('', '_blank');
...
function exportSerialHistoryPdf() {
  ...
  html += `</tbody></table><div class="atmec-print-footer"><span>Generato da AT-MEC HM 4.12</span><span>${new Date().toLocaleString('it-IT')}</span></div></body></html>`;
  const w = window.open('', '_blank');
  if (!w) { downloadTextFile(`storico_${serial}.html`, html, 'text/html'); return; }
  w.document.write(html); w.document.close(); setTimeout(() => { try { w.print(); } catch {} }, 350);
}
...
// AT-MEC_HM_4.12A - Analisi Produzione separata.
...
async function loadProductionAnalysisDashboard412A(){
  ...
  try{
    ...
    console.error('[AT-MEC 4.12A] Analisi Produzione', e);
  }
}
