// AT-MEC_HM 4.22 - Enterprise Stable Center
(function(){
  'use strict';
  var VERSION='AT-MEC_HM_4.22_ENTERPRISE_STABLE';
  ...
  var BACKUP_KEYS=[
    ...
    'atmec60_customers','atmec60_products','atmec60_workorders','atmec60_selected_workorder','atmec60_testmode_context',
    'atmec61_firmware','atmec61_revisions','atmec61_bindings','atmec61_change_history','atmec61_manufacturing_mode'
  ];
  ...
  function toast(m,t){if(window.toast) window.toast(m,t||'info'); else console.log('[4.22]',m)}
  ...
  window.runEnterpriseAudit422=function(){
    var checks=[];
    checks.push(check('Versione renderer', window.AT_MEC_RELEASE===VERSION || /4\.22/.test(document.title), 'release '+(window.AT_MEC_RELEASE||document.title)));
    ...
    checks.push(check('Work Orders', typeof window.showWorkOrderProduct60==='function' || localStorage.getItem('atmec60_workorders')!==null, '6.0 Work Orders'));
    checks.push(check('Firmware/Revisions', typeof window.showRevisionFirmware61==='function' || localStorage.getItem('atmec61_bindings')!==null, '6.1 Revision Firmware'));
    checks.push(check('Manufacturing Mode opzionale', localStorage.getItem('atmec61_manufacturing_mode')!==null, 'flag presente o inizializzabile', localStorage.getItem('atmec61_manufacturing_mode')===null));
  ...
  window.exportEnterpriseAudit422=function(){download('AT_MEC_HM_4_22_ENTERPRISE_AUDIT_'+Date.now()+'.json', window.__atmec422LastAudit||runEnterpriseAudit422());};
  window.exportEnterpriseBackup422=function(){
    ...
    download('AT_MEC_HM_4_22_BACKUP_'+Date.now()+'.json', payload);
  ...
  window.addEnterpriseLog422=function(){addEnterpriseLogInternal422(($('enterprise422-log-category')||{}).value||'SYSTEM',(($('enterprise422-log-message')||{}).value||'').trim()||'Log manuale 4.22'); if($('enterprise422-log-message')) $('enterprise422-log-message').value='';};
  ...
  window.clearEnterpriseLogs422=function(){if(confirm('Svuotare log enterprise 4.22?')){saveLogs([]);renderEnterpriseLogs422();}};
